...
def AlgorithmMSP_Sequential(A: list[int], N: int, M: int):
    ...
    for i in range(N):      # For each column in the array
        ...
        temp = [0] * N      # Creates empty array the size of the Y-axis
...
for i in range(len(x_steps)):       # For each matrix size/step option
    ...
    # npArray.append(np.random.randint(min_value, max_value + 1, size = (x_steps[i], y_steps[i])))      # Creating our two-dimentional array to search through
    array.append(matrixGenerator(x_steps[i]))
...
for i in range(len(x_steps)):       # For each matrix size/step option
    ...
    for j in range(iterations):     # Run each step of matrix-size for the set amount of iterations
        ...
        # printArray = []
        # for k in range(x_steps[i]):
        #     for l in range(len(array[i][k])):
        #         if array[i][k][l] <= 0:
        #             # print(array[i][k][l], end='')
        #             printArray[k].append(str(array[i][k][l]))
        #         else:
        #             printArray[k].append(" " + str(array[i][k][l]))
                    # print(" " + str(array[i][k][l]), end='')

                # print("\n")
                    
        # print(printArray)                    # Print given array for troubleshooting purposes
        # print(array[i])                    # Print given array for troubleshooting purposes
