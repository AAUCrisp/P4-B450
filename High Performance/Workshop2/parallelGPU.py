...
@cuda.jit
def blockPartitioning(f, blockPartition):
    tx = cuda.threadIdx.x
    ty = cuda.threadIdx.y
    bx = cuda.blockIdx.x
    by = cuda.blockIdx.y
    ...
    blockPartition[bx][by][tx][ty] = f[tx + (bd*bx)][ty + (bd*by)]
...
@cuda.jit
def quantizise(blockPartitionQuant, blockPartitionDCT, quantization_matrix):  
    tx = cuda.threadIdx.x
    ty = cuda.threadIdx.y
    bx = cuda.blockIdx.x
    by = cuda.blockIdx.y
    
    blockPartitionQuant[bx][by][tx][ty] = int(round(blockPartitionDCT[bx][by][tx][ty] / quantization_matrix[tx][ty]))



@cuda.jit
def DCT3_Coef(blockPartitionQuant, blockPartitionDCT3, quantization_matrix):
    tx = cuda.threadIdx.x
    ty = cuda.threadIdx.y
    bx = cuda.blockIdx.x
    by = cuda.blockIdx.y
    blockPartitionDCT3[bx][by][tx][ty] = blockPartitionQuant[bx][by][tx][ty] * quantization_matrix[tx][ty]
...
@cuda.jit
def partitionToImageArray(partition, retImage):
    tx = cuda.threadIdx.x
    ty = cuda.threadIdx.y
    bx = cuda.blockIdx.x
    by = cuda.blockIdx.y
    bd = cuda.blockDim.x
    retImage[tx + (bd*bx)][ty + (bd*by)] = partition[bx][by][tx][ty]
...
if __name__ == '__main__':
    ...
    griddim = (blockPartition.shape[0], blockPartition.shape[1])
    print(griddim)
    ...
    ########################################
    ######################################## 
    blockPartitioning[griddim, blockdim](f, blockPartition)
    ...
    partitionToImageArray[griddim, blockdim](blockPartitionDCT, fDCT)
    cuda.synchronize()
    ...
    quantizise[griddim, blockdim](blockPartitionQuant, blockPartitionDCT, quantization_matrix)
    cuda.synchronize()

    # Make printable version of the quantizised version
    fQuant = np.zeros((imageDimensions[0], imageDimensions[1]))
    partitionToImageArray[griddim, blockdim](blockPartitionQuant, fQuant)
    cuda.synchronize()
    ...
    DCT3_Coef[griddim, blockdim](blockPartitionQuant, blockPartitionDCT3, quantization_matrix)
    cuda.synchronize()
    ...
    partitionToImageArray[griddim, blockdim](blockPartitionRestored, fRestored)
    cuda.synchronize()
