...
char *Timestamp2() {
    ...
    sprintf(curr_time2, "[%d:%d:%d.%03ld %d/%d/%d]", timeinfo2->tm_hour, timeinfo2->tm_min, timeinfo2->tm_sec, tv2.tv_usec / 1000, timeinfo2->tm_mday, timeinfo2->tm_mon + 1, timeinfo2->tm_year + 1900);
...
char *processData(char msg[buffer]) {
    ...
    if (movement_x == 0 && movement_y == 0) {
        ...
        sprintf(coordinates, "%s\n%s %s\n%s %d %s\n%s %d %s\n", curr_time2, "Command read:", msg, "Movement on the x-axis:", movement_x, "mm", "Movement on the y-axis:", movement_y, "mm");
        // printf("%s\n", coordinates);
    }
